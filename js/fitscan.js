...
const notifier = nodeRequire('node-notifier');
const path = nodeRequire('path');

var fatbeeIconPath = path.join(__dirname, 'fatbee.png');
...
function notify(text) {
  notifier.notify({
    ...
    'message': text,
    'icon': fatbeeIconPath,
    'appIcon': fatbeeIconPath,
    sound: true
  });
  
}

electron.ipcRenderer.on('clipboard-change', function(event, message) {
...
function copyFitToClipboard() {
  clipboard.writeText($('#textFit').val());
  notify('Fit copied to clipboard.')
}
